...
export function VideoAvatarClient() {
  ...
  // Sign-language mode (?sign=asl): render the Signapse ASL panel in the avatar slot.
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      const signParam = (params.get("sign") || "").toLowerCase();
      const signOn = signParam === "asl" || signParam === "1" || signParam === "true";
      if (signOn) setSignMode(true);
      if (urlProfile) {
        setProfile(urlProfile);
      } else if (signOn) {
        // voice-only profile so the avatar slot is free for the sign panel
        setProfile("PREMIUM");
      }
                ...
                <div className="flex flex-col h-full">
                  ...
                  {showThymiaPanel || showShenPanel ? (
                    ...
                    <div className="flex-1 flex items-center justify-center bg-muted/20 p-2">
                      {signMode ? (
                        <SignPanel messages={messageList} isAgent={isAgentMessage} />
                      ) : (
                        <AvatarVideoDisplay
                          videoTrack={avatarVideoTrack}
