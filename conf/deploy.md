...
| cards | 8089 | `/home/ubuntu/web/cards` | Vite SPA from [BenWeekes/cards](https://github.com/BenWeekes/cards) — poker card renderer + `voice-agent/` sub-page. Backs the **Poker Dealer** demo (`/dealer`). PM2 runs `npx vite preview --port 8089 --strictPort --host 127.0.0.1`; the built `dist/` was produced with `VITE_BASE=/dealer/ npm run build` (see `vite.config.js` — reads the env). nginx `location = /dealer { return 301 /dealer/voice-agent/; }` sends the bare URL to the voice-agent entrypoint; `location ^~ /dealer/` proxies through to :8089. Backend URL is resolved at runtime by `src/voice-agent/voice-agent.js` — prod hostnames get same-origin `"/simple-backend"`. Uses the shared simple-backend (`POKER_*` env vars added to its `.env` with `POKER_ASR_VENDOR=ares`, `POKER_TTS_VENDOR=gradium` and a dedicated Gradium key). Rebuild flow: `git pull && npm install --legacy-peer-deps && VITE_BASE=/dealer/ npm run build && pm2 restart cards`. `--legacy-peer-deps` is required because `agora-rtm@2.2.4` pins `agora-rtc-sdk-ng@"4.23.0"` while the app's package.json floats `^4.23.0` → 4.24.5. |
