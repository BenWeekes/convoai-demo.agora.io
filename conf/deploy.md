...
| Photo Avatar — GRADIUMDEMO | https://convoai-demo.agora.io/photo?profile=GRADIUMDEMO | Photo demo variant with a cascading Gradium pipeline (OpenAI gpt-4o-mini + Deepgram Flux + **Gradium TTS**) + LemonSlice avatar (aspect_ratio=1x1). Frontend (`react-photo-avatar/lib/photo.ts::pickGradiumVoice`) resolves stock male/female Gradium voice from `analysis.sex` (male `_6Aslh2DxfmnRLmP`, female `cLONiZ4hQ8VpQ4Sz`) and passes it to `/start-agent` as `voice_id`. Backend `.env` block `GRADIUMDEMO_*` — TTS_VENDOR=gradium, key routed at run time. Prompt is a fixed event-specific script ("Raise AI Summit"), greeting "Hi there and welcome to Raise AI Summit". Isolated gallery at `/uploads/GRADIUMDEMO/`. 5 min cap. |
| Photo Avatar — GRADIUMDEMO + voice clone | https://convoai-demo.agora.io/photo?profile=GRADIUMDEMO&audiopick=GRADIUM | Same profile as above but adds the voice-clone step: after selecting a photo, the primary CTA changes from "💬 Talk" to "🎤 Pick voice" and routes through `/photo/voice-picker`. That page lets the user (a) record a new clone (MediaRecorder → POST `/simple-backend/clone-voice` → Gradium `POST /api/voices/` returns a `uid` used as voice_id), (b) select any of the last 12 clones from a datetime-labelled gallery with inline audio previews, or (c) skip and use the stock male/female fallback. Clone samples land at `/uploads/GRADIUMDEMO/voices/<slug>.{wav,json}` (slug = UTC datetime `YYYY-MM-DD-HHMMSS`). Sidecar shape: `{id, voice_id, vendor, created_at, created_at_iso, sample_bytes, sample_mime, sample_url, note}`. Auth header for Gradium is `x-api-key: <TTS_KEY>` (probed live; docs are Python-SDK only). Success detection uses `uid` presence — Gradium always returns a "wait a bit till the voice is ready" note even on 201-created, which is surfaced as `sidecar.note` rather than treated as failure. |
...
| Poker Dealer (voice-agent) | https://convoai-demo.agora.io/dealer | nginx 301 → `/dealer/voice-agent/`. Vite-built voice agent from [BenWeekes/cards](https://github.com/BenWeekes/cards) — ask for two cards ("ace of clubs and king of hearts") and it deals them onto a Three.js poker table. Uses `POKER` profile (gpt-4o-mini + ElevenLabs male voice + Ares ASR + aivad). Prompt instructs the LLM to append `{DEAL:XY,XY}` after confirmation; frontend parses the tag and animates the reveal. |
| Poker Dealer — Booth Viewer | https://convoai-demo.agora.io/dealer/viewer/ | Booth-mode "audience view" of the dealer that hides the ConvoAI internals. On load, POSTs to `/dealer/api/publisher/start`; cards Node ensures a headless Chrome (running the voice-agent in `pub_mode=1`) is publishing card-table video + agent TTS audio into the shared Agora channel `cards`, then hands the viewer their own tokens so they can join, publish mic, and see/hear the dealer. **Singleton publisher** — one headless Chrome serves N viewers. 15 min TTL per publisher birth from the GPU-Chrome service's `CHROME_URL_TTL_SECS`; next viewer request after expiry auto-relaunches. Publisher runs on `3.9.234.40 /publisher/chrome/open` (see `server/chrome_client.js` + cards `.env` `PUBLISHER_API_KEY`). Publisher endpoints live entirely in cards (not simple-backend) — see `publisher_plan.md` in the cards repo. No auth, booth-only. |
...
| cards | 8089 | `/home/ubuntu/web/cards` | Vite SPA from [BenWeekes/cards](https://github.com/BenWeekes/cards) — poker card renderer + `voice-agent/` sub-page + `viewer/` booth page. Backs the **Poker Dealer** demo (`/dealer`) and the **Poker Dealer Booth Viewer** (`/dealer/viewer/`). **PM2 now runs `node server/index.js`** (was `npx vite preview`); the Node server serves the built `dist/` statically AND the new `/api/publisher/{start,status}` endpoints. Built via `VITE_BASE=/dealer/ npm run build`. nginx `location = /dealer { return 301 /dealer/voice-agent/; }` sends the bare URL to the voice-agent entrypoint; `location ^~ /dealer/` proxies everything through to :8089 including `/dealer/api/*` (the Node server accepts both `/api/*` and `/dealer/api/*` since nginx forwards the full path). Backend URL for browser code is resolved at runtime by `src/voice-agent/voice-agent.js` — prod hostnames get same-origin `"/simple-backend"`. Uses the shared simple-backend (`POKER_*` env vars in simple-backend `.env`). The booth flow needs additional `.env` in `cards/`: `POKER_APP_ID`, `POKER_APP_CERTIFICATE` (duplicated from simple-backend so cards mints its own tokens), `POKER_DEALER_CHANNEL=cards`, `PUBLISHER_SERVICE_URL=http://3.9.234.40/publisher`, `PUBLISHER_API_KEY`, `SIMPLE_BACKEND_URL=http://127.0.0.1:8082`, `PUBLISHER_URL_BASE=https://convoai-demo.agora.io`. Rebuild flow: `git pull && npm install --legacy-peer-deps && VITE_BASE=/dealer/ npm run build && pm2 restart cards`. `--legacy-peer-deps` is required because `agora-rtm@2.2.4` pins `agora-rtc-sdk-ng@"4.23.0"` while the app's package.json floats `^4.24.2` → 4.24.5. Publisher endpoints call `3.9.234.40 /publisher/chrome/open` (x-api-key auth) to spawn a headless Chrome loading `/dealer/voice-agent/?pub_mode=1&…`; that Chrome joins Agora as uid 200 and publishes canvas.captureStream(30) — see `publisher_plan.md`. |
...
client sends `[reveal]`. Full design + test guide: `baccarat/baccarat_plan.md`.

- **Service:** `baccarat-mcp` (pm2), Node streamable-HTTP MCP on `127.0.0.1:8117`, nginx
  `location /baccarat-mcp/` → 8117. **Source lives in THIS repo at `baccarat/mcp/`**
  ...
  **symlink** to it, so pm2's cwd and the hardcoded log paths keep working. `node_modules`
  ...
  `/home/ubuntu/baccarat-mcp → baccarat/mcp`, `npm i`, then the pm2 start below.
